# AdventOfCode 2017 Day 4 Part 2
...
const
    phrase1: string = "abcde fghij"
    phrase2: string = "abcde xyz ecdab"
    phrase3: string = "a ab abc abd abf abj"
    phrase4: string = "iiii oiii ooii oooi oooo"
    phrase5: string = "oiii ioii iioi iiio"
    phrases: string = """
        abcde fghij
        abcde xyz ecdab
...
proc check_phrase(phrase: string): bool =
    var words: seq[string] = splitWhitespace(phrase)
    words.applyit(sorted(it, system.cmp).join)
    let wordset: HashSet[string] = toSet(words)
    return len(words) == len(wordset)

proc number_valid_phrases(phrasestring: string): int =
    # Count the number of valid phrases in the phrasestring and return the total
    let phrases: seq[string] = splitLines(phrasestring)
    for phrase in phrases:
        if check_phrase(phrase):
            result += 1

echo "Phrase1: ", check_phrase(phrase1) == true
echo "Phrase2: ", check_phrase(phrase2) == false
echo "Phrase3: ", check_phrase(phrase3) == true
echo "Phrase4: ", check_phrase(phrase4) == true
echo "Phrase5: ", check_phrase(phrase5) == false
echo "Sum: ", number_valid_phrases(phrases) == 3
...
echo "Number of valid phrases: ", number_valid_phrases(puzzle)