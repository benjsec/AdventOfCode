...
proc calc_weight(stack: var Table, prog: Program): int =
    result = prog.weight
    for child in prog.children:
        result += stack.calc_weight(stack[child])

proc check_even(stack: var Table, prog: Program): bool =
...
proc inbalance_search(stack: var Table, head: string=nil) =
    # Find head then do a depth-first search and print
    # all nodes with unbalanced stacks.
    var h: string = if head == nil: find_head(stack) 
            else: head
    # echo "\n", h
    if check_even(stack, stack[h]) == false:
